...
async function runBenchmark() {
  ...
  for (const task of BENCHMARK_TASKS) {
    ...
    console.log(`   ✓ Completed in ${withoutResult.latency}ms (cost: $${withoutResult.cost.toFixed(3)})`);
    ...
    console.log(`   ✓ Completed in ${withResult.latency}ms (cost: $${withResult.cost.toFixed(3)}, confidence: ${((withResult.confidence || 0) * 100).toFixed(1)}%)`);
  ...
  console.log('┌─────────────────────────────┬──────────────────┬──────────────────┬─────────────┐');
  console.log('│ Metric                      │ Standard Agent   │ Agent + Booster  │ Improvement │');
  console.log('├─────────────────────────────┼──────────────────┼──────────────────┼─────────────┤');
  console.log(`│ Avg Latency                 │ ${String(avgLatencyWithout).padStart(11, ' ')}ms    │ ${String(avgLatencyWith.toFixed(0)).padStart(11, ' ')}ms    │ ${avgSpeedup}x faster │`);
  console.log(`│ Total Cost (${BENCHMARK_TASKS.length} edits)        │ $${String(totalCostWithout.toFixed(2)).padStart(13, ' ')} │ $${String(totalCostWith.toFixed(2)).padStart(13, ' ')} │ ${costSavings}% savings │`);
  console.log(`│ Success Rate                │ ${String((results.withoutBooster.filter(r => r.success).length / results.withoutBooster.length * 100).toFixed(0)).padStart(12, ' ')}%    │ ${String((results.withBooster.filter(r => r.success).length / results.withBooster.length * 100).toFixed(0)).padStart(12, ' ')}%    │ Comparable  │`);
  console.log('└─────────────────────────────┴──────────────────┴──────────────────┴─────────────┘');

  console.log('\n\n🎯 Key Takeaways:\n');
  console.log(`   • Agent Booster provides ${avgSpeedup}x speedup for code editing tasks`);
  console.log(`   • Saves $${totalCostWithout.toFixed(2)} per ${BENCHMARK_TASKS.length} edits (${costSavings}% cost reduction)`);
  console.log(`   • Perfect for: agentic workflows, autonomous refactoring, CI/CD pipelines`);
  console.log(`   • Integration: Use --agent-booster flag with agentic-flow claude-code`);
