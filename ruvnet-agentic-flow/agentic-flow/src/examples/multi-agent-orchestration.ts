...
async function main() {
  ...
  if (goalPlanner) {
    ...
    const planResult = await claudeAgent(
      goalPlanner,
      'Create a 3-step plan to improve our Claude Agent SDK implementation with error handling, logging, and monitoring.'
    );
    console.log('\n✅ Plan created\n');
  ...
  if (codeAnalyzer) {
    ...
    const analysisResult = await claudeAgent(
      codeAnalyzer,
      'Analyze the code quality of a Claude Agent SDK implementation focusing on error handling patterns and logging.'
    );
    console.log('\n✅ Analysis complete\n');
