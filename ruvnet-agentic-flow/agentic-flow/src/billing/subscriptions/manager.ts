...
export class SubscriptionManager extends EventEmitter {
  ...
  async renewSubscription(subscriptionId: string): Promise<Subscription> {
    ...
    if (subscription.metadata?.pendingDowngrade) {
      const newTier = subscription.metadata.pendingDowngrade as SubscriptionTier;
      const newPricing = this.pricingManager.getTier(newTier);
   ...
   */
  async hasAccess(subscriptionId: string, feature: string): Promise<boolean> {
    const subscription = await this.storage.getSubscription(subscriptionId);
  ...
  private getPriceForCycle(tier: SubscriptionTier, cycle: BillingCycle): number {
    const cycleMap = {
