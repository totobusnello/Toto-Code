    ...
    for (const result of this.results) {
      if (result.improvement.successRatePercent < 0) {
        recommendations.push(
          `⚠️  ${result.scenarioName}: Baseline outperformed ReasoningBank - review memory quality`
        );
      }

      if (result.improvement.tokenSavings < 0) {
        recommendations.push(
          `💡 ${result.scenarioName}: Token overhead detected - optimize memory retrieval (k parameter)`
        );
      }

      if (result.improvement.latencyDelta > 1000) {
        recommendations.push(
          `⚡ ${result.scenarioName}: High latency - consider caching or reducing consolidation frequency`
        );
      }
    }
