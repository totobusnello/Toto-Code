...
import type { BenchmarkReport, ScenarioResults } from './types.js';
...
function generateScenarioSection(scenario: ScenarioResults): string {
...
function generateDifficultyDistribution(scenario: ScenarioResults): string {
  // This would need access to original tasks, simplified for now
  return 'Easy: 3, Medium: 5, Hard: 2';
}

function getStatusEmoji(value: number, type: 'success' | 'token' | 'latency' | 'learning'): string {
  switch (type) {
    case 'success':
    ...
    case 'learning':
      ...
      if (value >= 1) return '📊 Modest';
      return '⚠️ Slow';
  }
}

function generateLearningCurve(scenario: ScenarioResults): string {
...
|-----------|------------------|----------------------|-----------|----------|
${scenario.learningCurve.map(point =>
  `| ${point.iteration} | ${(point.baselineSuccess * 100).toFixed(1)}% | ${(point.reasoningbankSuccess * 100).toFixed(1)}% | ${Math.round(point.reasoningbankTokens).toLocaleString()} | ${point.memoriesAvailable} |`
).join('\n')}
...
function generateLearningInsights(learningCurve: any[]): string {
