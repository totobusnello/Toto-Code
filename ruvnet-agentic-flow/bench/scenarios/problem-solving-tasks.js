...
Example: [2, 7, 11, 15], target = 9 → [0, 1] (because 2 + 7 = 9)`,
        successCriteria: (result) => {
            ...
            return ((code.includes('map') || code.includes('set') || code.includes('object')) &&
...
Example: "abcabcbb" → 3 (substring "abc")`,
        successCriteria: (result) => {
            const code = result.output.toLowerCase();
            return ((code.includes('set') || code.includes('map')) &&
...
Example: [[1,3],[2,6],[8,10],[15,18]] → [[1,6],[8,10],[15,18]]`,
    ...
    },
    {
        ...
        successCriteria: (result) => {
            const code = result.output.toLowerCase();
            return ((code.includes('bfs') || code.includes('queue')) &&
    ...
    },
    {
        ...
        successCriteria: (result) => {
            const code = result.output.toLowerCase();
            return ((code.includes('dp') || code.includes('dynamic')) &&
    ...
    },
    {
        ...
        successCriteria: (result) => {
            const code = result.output.toLowerCase();
            return (code.includes('serialize') &&
...
Example: [0,1,0,2,1,0,1,3,2,1,2,1] → 6 units of water`,
        successCriteria: (result) => {
            const code = result.output.toLowerCase();
            return ((code.includes('left') && code.includes('right')) &&
    ...
    },
    {
        ...
        successCriteria: (result) => {
            const code = result.output.toLowerCase();
            return ((code.includes('dp') || code.includes('dynamic') || code.includes('recursive')) &&
...
export const problemSolvingTasksScenario = {
