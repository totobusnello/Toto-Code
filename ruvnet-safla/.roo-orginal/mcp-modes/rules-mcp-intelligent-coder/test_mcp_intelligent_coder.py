...
class TestMCPIntelligentCoderMode:
    ...
    async def test_error_handling_and_fallback(self, workflow, mcp_client):
        ...
        async def failing_mcp_tool(server_name, tool_name, arguments):
            if server_name == "context7":
                raise Exception("Context7 server unavailable")
            return await mcp_client.use_mcp_tool(server_name, tool_name, arguments)
        
        # Replace MCP client method with failing version
        original_method = mcp_client.use_mcp_tool
        mcp_client.use_mcp_tool = failing_mcp_tool
        ...
        try:
            ...
            mcp_client.use_mcp_tool = original_method
